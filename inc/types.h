...
struct kMethodCandiScale
{
    unsigned k;
    unsigned methodID; // {"top", "down", "mid", "rand"};
    double candiScale;
    double rate; //(nlf-lmtf)/nlf; read only on print stage

    bool operator<(const kMethodCandiScale &r) const
    {
        if (candiScale != r.candiScale)
        {
            return candiScale < r.candiScale;
        }
        return k < r.k;
    }
};
